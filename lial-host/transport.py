"""
LIAL Transport Abstraction Layer.

Defines a common interface for all transport backends (serial, TCP, BLE)
so the host can communicate with any LIAL receiver regardless of physical link.
"""
...
class LialTransport(ABC):
    """Abstract base class for LIAL-Link transports."""

    @abstractmethod
    def read_frame(self, timeout: float = 30.0) -> tuple[int, bytes]:
        """Read one LIAL-Link frame. Returns (opcode, payload).
...
class SerialTransport(LialTransport):
    """LIAL-Link over USB serial (pyserial)."""
...
class TcpTransport(LialTransport):
    """LIAL-Link over persistent TCP connection."""
