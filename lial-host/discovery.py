"""
LIAL Device Discovery via mDNS and BLE.

Discovers already-flashed LIAL receivers on the local network using mDNS
(service type `_lial._tcp.local.`) or via BLE advertising.
"""
...
class DiscoveredDevice:
    """A LIAL receiver found via network discovery."""
...
LIAL_MDNS_SERVICE = "_lial._tcp.local."


def discover_mdns(timeout: float = 5.0) -> list[DiscoveredDevice]:
    """Discover LIAL devices via mDNS. Requires `zeroconf` package.
    ...
    class Listener:
        def add_service(self, zc: Zeroconf, type_: str, name: str) -> None:
            ...
            dev = DiscoveredDevice(
                name=name.replace(f".{LIAL_MDNS_SERVICE}", ""),
                host=addresses[0],
    ...
    zc = Zeroconf()
    listener = Listener()
    browser = ServiceBrowser(zc, LIAL_MDNS_SERVICE, listener)
