...
from flash_backends import esp32 as esp32_backend
from flash_backends import rp2040 as rp2040_backend
from flash_backends import avr as avr_backend
from flash_backends import stm32 as stm32_backend
...
def test_esp32_probe_parses_chip_type():
    ...
    with patch("flash_backends.esp32.subprocess.run", return_value=_fake_run_ok(stdout=stdout)):
...
def test_esp32_probe_failure_returns_reason():
    with patch("flash_backends.esp32.subprocess.run", return_value=_fake_run_ok(returncode=2, stderr="no serial data\n")):
        b = esp32_backend.Esp32Backend(esptool_cmd=["esptool"])
        result = b.probe("/dev/ttyUSB0")
...
def test_esp32_flash_invokes_write_flash_with_offset(tmp_path):
    firmware = tmp_path / "lial-receiver-esp32c3.bin"
    ...
    with patch("flash_backends.esp32.subprocess.run", side_effect=_fake_run):
        b = esp32_backend.Esp32Backend(esptool_cmd=["esptool"])
...
def test_esp32_flash_handles_esptool_not_installed(tmp_path):
    ...
    with patch("flash_backends.esp32.subprocess.run", side_effect=FileNotFoundError("esptool")):
        b = esp32_backend.Esp32Backend(esptool_cmd=["esptool"])
        result = b.flash("/dev/ttyUSB0", str(firmware), variant="esp32c3")
    assert not result.ok
...
def test_rp2040_probe_detects_bootsel_mount(tmp_path):
    ...
    with patch("flash_backends.rp2040._find_bootsel_mount", return_value=mount):
...
def test_rp2040_flash_no_mount_and_no_picotool(tmp_path):
    firmware = tmp_path / "fw.uf2"
    firmware.write_bytes(b"UF2\n")
    with patch("flash_backends.rp2040._find_bootsel_mount", return_value=None), \
         patch("flash_backends.rp2040._picotool_available", return_value=False):
        b = rp2040_backend.Rp2040Backend()
        result = b.flash("/dev/whatever", str(firmware))
    assert not result.ok
...
def test_avr_probe_detects_atmega328p():
    ...
    with patch("flash_backends.avr.subprocess.run", side_effect=fake_run):
...
def test_stm32_probe_detects_dfu():
    ...
    with patch("flash_backends.stm32.shutil.which", return_value="/usr/bin/dfu-util"), \
         patch("flash_backends.stm32.subprocess.run", return_value=_fake_run_ok(stdout=dfu_stdout)):
...
def test_stm32_flash_dfu(tmp_path):
    ...
    with patch("flash_backends.stm32.shutil.which", return_value="/usr/bin/dfu-util"), \
         patch("flash_backends.stm32.subprocess.run", side_effect=fake_run):
        b = stm32_backend.Stm32Backend()
        result = b.flash("/dev/whatever", str(firmware))
    assert result.ok
...
def test_registry_lazy_registers_all_backends():
    names = flash_backends.available_backends()
    for expected in ("esptool", "uf2", "avrdude", "stm32loader"):
        assert expected in names


def test_get_backend_returns_correct_type():
    assert isinstance(flash_backends.get_backend("esptool"), esp32_backend.Esp32Backend)
    assert isinstance(flash_backends.get_backend("uf2"), rp2040_backend.Rp2040Backend)
    assert isinstance(flash_backends.get_backend("avrdude"), avr_backend.AvrBackend)
    assert isinstance(flash_backends.get_backend("stm32loader"), stm32_backend.Stm32Backend)
    assert flash_backends.get_backend("nonexistent") is None
