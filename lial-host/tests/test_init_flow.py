...
def test_init_reports_nothing_when_no_ports():
    ...
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[]):
...
def test_init_skips_ports_with_unknown_vidpid(capsys):
    ...
    args = argparse.Namespace(port=None, yes=True, manifest_url="x", cache_dir="/tmp", dry_run=True)
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[unknown]):
        rc = init_cmd.run(args)
...
def test_init_dry_run_lists_targets(capsys):
    ...
    args = argparse.Namespace(port=None, yes=True, manifest_url="x", cache_dir="/tmp", dry_run=True)
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[dev]), \
         patch("lial_commands.init._probe_lial_firmware", return_value=None):
        rc = init_cmd.run(args)
    assert rc == 0
    out = capsys.readouterr().out
...
def test_init_skips_ports_with_lial_already_flashed(capsys):
    dev = _make_device("/dev/cu.usbmodem1")
    args = argparse.Namespace(port=None, yes=True, manifest_url="x", cache_dir="/tmp", dry_run=True)
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[dev]), \
         patch("lial_commands.init._probe_lial_firmware", return_value="0.2.0"):
        rc = init_cmd.run(args)
    out = capsys.readouterr().out
    assert rc == 0
    assert "already running LIAL v0.2.0" in out
...
def test_init_full_flow_flashes_blank_board(tmp_path):
    ...
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[dev]), \
         patch("lial_commands.init._probe_lial_firmware", return_value=None), \
         patch("lial_commands.init.get_backend", return_value=fake_backend), \
         patch("lial_commands.init.fetch_manifest", return_value=manifest), \
         patch("lial_commands.init.download_firmware") as fake_dl:
    ...
    assert args_positional[1].endswith("lial-receiver-esp32c3.bin")
...
def test_init_filters_by_port(capsys):
    ...
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[d1, d2]), \
         patch("lial_commands.init._probe_lial_firmware", return_value=None):
        rc = init_cmd.run(args)
    out = capsys.readouterr().out
    assert rc == 0
...
def test_init_missing_port_errors(capsys):
    ...
    with patch("lial_commands.init.enumerate_usb_devices", return_value=[d1]):
        rc = init_cmd.run(args)
    assert rc == 1
    assert "no device found" in capsys.readouterr().err


def test_probe_lial_firmware_parses_discovery_frame():
    ...
    with patch("serial.Serial", return_value=fake_serial):
        version = init_cmd._probe_lial_firmware("/dev/fake", timeout_s=0.5)
    assert version == "0.2.0"


def test_probe_lial_firmware_returns_none_on_silence():
    fake_serial = MagicMock()
    fake_serial.read.return_value = b""
    with patch("serial.Serial", return_value=fake_serial):
        version = init_cmd._probe_lial_firmware("/dev/fake", timeout_s=0.1)
...
def test_pick_family_ambiguous_picks_probing_winner():
    ...
    with patch("lial_commands.init.get_backend", side_effect=get_backend):
