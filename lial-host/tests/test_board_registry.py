...
def test_detected_device_states():
    # Fresh blank ESP32: candidates matched, no LIAL firmware.
    ...
    assert d.has_lial is False
    assert d.is_blank is True

    # Same device, already running LIAL.
    d.lial_version = "0.2.0"
    assert d.has_lial is True
    assert d.is_blank is False

    # Unknown USB device.
    d2 = br.DetectedDevice(port="/dev/cu.usbmodem2", vid=0xDEAD, pid=0xBEEF,
                           candidate_families=[])
    assert d2.has_lial is False
