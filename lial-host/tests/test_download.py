    ...
    "variants": [
        {
            ...
            "filename": "lial-receiver-esp32c3-0.2.0.bin",
            "url": "https://example.com/lial-receiver-esp32c3-0.2.0.bin",
            "size_bytes": 4096,
            "sha256": "a" * 64,
            "flash_tool": "esptool",
        },
        {
            ...
            "filename": "lial-receiver-rp2040-0.2.0.uf2",
            "url": "https://example.com/lial-receiver-rp2040-0.2.0.uf2",
            "size_bytes": 6144,
        },
    ],
}
...
def test_fetch_manifest_parses_json():
    ...
    with patch("lial_commands.download.urllib.request.urlopen", return_value=FakeResponse(body)):
...
def test_fetch_manifest_invalid_json_raises():
    with patch("lial_commands.download.urllib.request.urlopen", return_value=FakeResponse(b"not json")):
...
def test_run_with_explicit_board(tmp_path, capsys):
    ...
    with patch("lial_commands.download.urllib.request.urlopen", side_effect=fake_urlopen):
        rc = download_cmd.run(args)
    assert rc == 0
    downloaded = tmp_path / "lial-receiver-esp32c3-0.2.0.bin"
