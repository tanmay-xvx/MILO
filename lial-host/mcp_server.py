"""
LIAL MCP Server — expose LIAL as Model Context Protocol tools.

This server allows any MCP-compatible LLM agent (Claude, Cursor, etc.)
to discover, program, and control LIAL hardware devices via tool calls.

Tools exposed:
  - lial_devices: List connected devices and their capabilities
  - lial_push: Compile and push Rust code to a named device
  - lial_stop: Stop execution on a device
  - lial_query: Get device status
  - lial_set_param: Set a runtime parameter on a device
  - lial_hot_swap: Replace running code on a device
"""
...
def handle_tool_call(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    handlers = {
        "lial_devices": tool_lial_devices,
        "lial_push": tool_lial_push,
        "lial_stop": tool_lial_stop,
        "lial_query": tool_lial_query,
        "lial_set_param": tool_lial_set_param,
        "lial_hot_swap": tool_lial_hot_swap,
    }
...
def tool_lial_devices(args: dict) -> dict:
    """List all connected LIAL devices with their hardware manifests."""
    devices = registry.list_devices()
    return {"devices": devices, "count": len(devices)}


def tool_lial_push(args: dict) -> dict:
...
def tool_lial_stop(args: dict) -> dict:
...
def tool_lial_query(args: dict) -> dict:
...
def tool_lial_set_param(args: dict) -> dict:
...
def tool_lial_hot_swap(args: dict) -> dict:
...
MCP_TOOLS = [
    {
        "name": "lial_devices",
        "description": "List all connected LIAL hardware devices and their capabilities (pins, buses, memory).",
        "inputSchema": {
            "type": "object",
            "properties": {},
        },
    ...
    {
        "name": "lial_push",
        "description": "Compile Rust code to WebAssembly and push it to a specific LIAL device for execution. The code should use LIAL syscalls (lial_gpio_set, lial_delay_ms, lial_log, etc.).",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "lial_stop",
        "description": "Stop the currently running program on a LIAL device.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "device": {"type": "string", "description": "Name of the target device"},
    ...
    {
        "name": "lial_query",
        "description": "Query the execution status of a LIAL device (idle, running, completed, stopped).",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "lial_set_param",
        "description": "Set a runtime parameter on a LIAL device. The running Wasm module can read parameters via lial_get_param(slot). Useful for adjusting behavior without redeployment.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "device": {"type": "string", "description": "Name of the target device"},
    ...
    {
        "name": "lial_hot_swap",
        "description": "Stop the current program and immediately start new code on a LIAL device. Enables live code replacement without device reboot.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "device": {"type": "string", "description": "Name of the target device"},
