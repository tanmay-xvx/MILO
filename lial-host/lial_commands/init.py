"""`lial init` -- auto-detect connected boards and flash LIAL firmware.

Flow:
    1. Enumerate USB devices via board_registry.enumerate_usb_devices().
    2. For each device with a candidate family, check if it already runs LIAL
    ...
    4. For each confirmed target, call the family's FlashBackend.probe() to
       confirm variant, download the matching firmware via `lial download`,
       and flash.
"""
...
from flash_backends import FlashBackend, get_backend
from lial_commands.download import (
    DEFAULT_CACHE_DIR,
...
def add_parser(subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    p = subparsers.add_parser(
        "init",
        help="Auto-detect connected boards and flash LIAL firmware",
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    for dev in devices:
        ...
        # Is the device already running LIAL?
        lial_version = _probe_lial_firmware(dev.port)
        if lial_version is not None:
            dev.lial_version = lial_version
            print(f"  skip {dev.port}: already running LIAL v{lial_version}")
            continue
