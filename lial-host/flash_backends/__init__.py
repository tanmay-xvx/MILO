...
class FlashBackend(abc.ABC):
    ...
    name: str  # must match BoardFamily.flash_backend
...
def get_backend(name: str) -> FlashBackend | None:
    # Lazy import so adding a new backend never causes unrelated imports to fail.
    if not _BACKENDS:
        _lazy_register_all()
...
def _lazy_register_all() -> None:
    """Import and register every built-in backend. Failures are swallowed with a
    warning so a single missing dependency does not break all flashing."""
