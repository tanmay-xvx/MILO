"""
LIAL Device — high-level async interface for controlling a LIAL receiver.
...
class LialDevice:
    """High-level interface for a single LIAL receiver.

    Wraps a `LialTransport` and provides typed methods for all LIAL-Link
    opcodes including the extended control protocol.
    """

    def __init__(self, transport: LialTransport, name: str = "unnamed"):
    ...
    def set_param(self, slot: int, value: int) -> None:
        ...
        The running Wasm module can read this via `lial_get_param(slot)`.
        """
