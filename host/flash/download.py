"""`milo download` -- interactive firmware picker + fetcher.
...
Flow:
    ...
    5. Print the flash command they can run themselves (or chain into `milo init`).
"""
...
DEFAULT_MANIFEST_URL = os.environ.get(
    "MILO_MANIFEST_URL",
    "https://github.com/tanmay-xvx/MILO/releases/latest/download/manifest.json",
)

DEFAULT_CACHE_DIR = Path(os.environ.get("MILO_FIRMWARE_DIR", "./firmware"))


def add_parser(subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    p = subparsers.add_parser(
        "download",
        help="Download MILO receiver firmware for a board",
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    print(f"  saved: {dest}")
    flash_tool = variant.get("flash_tool", "milo init")
...
def interactive_pick(variants: list[dict]) -> dict | None:
    print()
    print("  Available MILO receiver firmware:")
    print()
