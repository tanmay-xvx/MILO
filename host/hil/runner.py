#!/usr/bin/env python3
"""Hardware-in-the-loop (HIL) test harness for MILO receiver firmware.

Compiles per-syscall Rust snippets to wasm, pushes them to a real ESP32-C3
(or any board running MILO), collects the on-device log output, and asserts
against it. Designed to run against physical hardware; there is no mocking
here -- that's what `receiver/tests/` covers.
...
Each test is a function in `host/hil/*.py` decorated with
...
# ── wire format helpers (duplicated from cli so this harness is self-contained) ──
...
def _load_all_tests() -> None:
    tests_dir = pathlib.Path(_HOST_DIR) / "hil"
    if not tests_dir.is_dir():
        return
    for f in sorted(tests_dir.glob("*.py")):
        if f.name.startswith("_"):
            continue
        module_name = f"hil.{f.stem}"
...
def main() -> int:
    ...
    if not _TESTS:
        print("  no HIL tests registered; drop files into host/hil/", file=sys.stderr)
        return 1
...
if __name__ == "__main__":
    ...
    # per-syscall scripts under `hil/` do `from hil.runner import ...`,
