//! Generic `embedded-hal`-backed implementation of `MiloHardware`.
//!
...
//!
//! The adapter exposes the full `MiloHardware` surface so it can be used
//! directly as the `H` parameter of `MiloRuntime<H>`.
...
/// rich error type into wasm -- the MILO syscall ABI only exposes an i32.
pub trait DynI2c {
...
/// UART bus. Separate from the MILO-Link transport: this is a user-facing
/// secondary UART exposed to wasm as `uart_*`.
pub trait DynUart {
...
/// `MiloHardware` implementation that composes boxed trait objects at runtime.
///
...
impl<'d> MiloHardware for EmbeddedHalAdapter<'d> {
    ...
    fn i2c_transfer(&mut self, addr: u8, tx: &[u8], rx: &mut [u8]) -> i32 {
        // Default to bus 0 -- we only expose `i2c_transfer(addr, ...)` in
