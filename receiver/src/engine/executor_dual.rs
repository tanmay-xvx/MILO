...
pub struct DualCoreExecutor<H: MiloHardware + 'static> {
    hardware: Option<H>,
    fuel: Option<u64>,
    status: ExecStatus,
...
impl<H: MiloHardware + 'static> DualCoreExecutor<H> {
...
impl<H: MiloHardware + 'static> MiloExecutor for DualCoreExecutor<H> {
    type Hardware = H;
    ...
    fn submit(&mut self, bytecode: &[u8]) {
        if !self.core1_available {
            ...
            let mut runtime = crate::MiloRuntime::new(hw, self.fuel);
