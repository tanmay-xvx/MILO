//! Execution engine abstraction for MILO.
//!
//! The `MiloExecutor` trait decouples Wasm execution from the main loop so
...
/// and the Wasm module (via `get_param` syscall).
...
pub trait MiloExecutor {
    /// The hardware type this executor uses.
    type Hardware: MiloHardware;
...
pub struct SingleCoreExecutor<H: MiloHardware + 'static> {
    hardware: Option<H>,
    fuel: Option<u64>,
    status: ExecStatus,
...
impl<H: MiloHardware + 'static> SingleCoreExecutor<H> {
...
impl<H: MiloHardware + 'static> MiloExecutor for SingleCoreExecutor<H> {
    type Hardware = H;
    ...
    fn submit(&mut self, bytecode: &[u8]) {
        ...
        let mut runtime = MiloRuntime::new(hw, self.fuel);
