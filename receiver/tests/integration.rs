use milo_receiver::{MiloError, MiloRuntime, targets::mock::LaptopMock};
use std::fs;
...
fn test_happy_path() {
    ...
    let mut runtime = MiloRuntime::new(hw, None);
    let logs = runtime.execute(&wasm, "run_logic").expect("should succeed");
    // mock_driver calls gpio_set and delay_ms -- no log_msg calls, so logs is empty
...
fn test_missing_export() {
    ...
    assert!(
        matches!(result, Err(MiloError::MissingExport(_))),
...
fn test_fuel_exhaustion() {
    ...
    let hw = LaptopMock::new();
    let mut runtime = MiloRuntime::new(hw, Some(10_000));
    let result = runtime.execute(&wasm, "run_logic");
    assert!(
        matches!(result, Err(MiloError::FuelExhausted)),
...
fn test_bad_module() {
    ...
    assert!(
        matches!(result, Err(MiloError::ModuleInvalid(_))),
