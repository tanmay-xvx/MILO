...
pub struct ModuleHeader {
    inner: Rc<ModuleHeaderInner>,
}

#[derive(Debug)]
struct ModuleHeaderInner {
    engine: EngineWeak,
    func_types: Rc<[DedupFuncType]>,
...
impl Module {
    ...
    /// # Note
    ///
    /// The slice is stored in a `Rc` so that this operation is very cheap.
    pub(crate) fn func_types_cloned(&self) -> Rc<[DedupFuncType]> {
