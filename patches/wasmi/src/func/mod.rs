...
impl<T> HostFuncTrampolineEntity<T> {
    ...
    pub fn new(
        // engine: &Engine,
        ty: FuncType,
        func: impl Fn(Caller<'_, T>, &[Val], &mut [Val]) -> Result<(), Error> + 'static,
    ) -> Self {
...
type TrampolineFn<T> =
    dyn Fn(Caller<T>, FuncInOut) -> Result<FuncFinished, Error> + 'static;

pub struct TrampolineEntity<T> {
    closure: Rc<TrampolineFn<T>>,
}
...
impl<T> TrampolineEntity<T> {
    ...
    where
        F: Fn(Caller<T>, FuncInOut) -> Result<FuncFinished, Error> + 'static,
    {
        Self {
            closure: Rc::new(trampoline),
        }
    }
