//! Integration tests that drive the full wasm pipeline (LialRuntime ->
...
use lial_receiver::embedded_hal_adapter::{
...
fn adapter_drives_real_wasm_mock_driver() {
    // mock_driver calls lial_gpio_set(5, ...) and lial_delay_ms repeatedly.
    let pin = SpyPin::new();
    ...
    let mut runtime = LialRuntime::new(adapter, None);
    let _ = runtime
...
fn adapter_i2c_path_works() {
    // Custom wasm that calls lial_i2c_transfer isn't in fixtures yet; instead
    ...
    use lial_receiver::LialHardware;
    let mut rx = [0u8; 3];
