...
//!
//! Implements `LialTransport` over a persistent TCP socket on port 9100.
//! The receiver acts as a TCP server: it listens for one client connection
//! and processes LIAL-Link frames over that socket.

use crate::link::{Frame, LinkError};
use crate::transport::LialTransport;
use alloc::string::String;
use alloc::vec;

/// TCP port for LIAL-Link WiFi transport.
pub const LIAL_TCP_PORT: u16 = 9100;
...
///
/// After WiFi association and DHCP, this listens on `LIAL_TCP_PORT` and
/// accepts a single client. Frames are read/written over the TCP stream
/// using the standard LIAL-Link 5-byte header protocol.
///
...
impl LialTransport for WifiTcpTransport {
